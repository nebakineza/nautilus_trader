#!/usr/bin/env python3
"""Run LeadLagMMv4 multi-symbol (latency optimized)."""
...
try:
    from strategy.lead_lag_bybit_binance_mm_v004 import LeadLagMMv4
    from strategy.lead_lag_bybit_binance_mm_v004 import LeadLagMMv4Config
except ModuleNotFoundError:
    sys.path.insert(0, "/home/ubuntu/trading")
    from lead_lag_bybit_binance_mm_v004 import LeadLagMMv4
    from lead_lag_bybit_binance_mm_v004 import LeadLagMMv4Config


def get_config(symbol: str, leader_id: str, guard_id: str | None, size: Decimal, max_size: Decimal) -> LeadLagMMv4Config:
    if symbol == "BTCUSDT":
        spread = Decimal("16.0")
        guard = Decimal("10.0")
...
def main() -> None:
    trader_id = TraderId("LEADLAG-MULTI-004")

    logging = LoggingConfig(
        log_level="INFO",
        log_level_file="INFO",
        log_directory=".",
        log_file_name="leadlag_multi_v4.log",
        clear_log_file=True,
    )
    ...
    for idx, pair in enumerate(pairs):
        ...
        node.add_strategy(LeadLagMMv4(config=config))
