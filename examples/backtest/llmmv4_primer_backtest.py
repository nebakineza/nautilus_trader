...
class PairConfig:
    ...
    internal_price_delta_limit: Decimal
...
DEFAULT_PAIRS = [
    ...
    PairConfig(
        ...
        min_order_qty=Decimal("80.0"),
        max_position_qty=Decimal("800.0"),
        guard_threshold_bps=Decimal("12.0"),
        spread_bps=Decimal("24.0"),
        min_profit_bps=Decimal("2.0"),
        refresh_interval_ms=5000,
        refresh_offset_ms=99,
        liquidity_high_qty=Decimal("5000.0"),
        liquidity_low_qty=Decimal("200.0"),
        ofi_enabled=True,
        ofi_max_bps=Decimal("5.0"),
...
def _compute_wap_from_fills(fills_report) -> dict:
    ...
    for _, row in fills_report.iterrows():
        ...
        fill_counts[inst] = fill_counts.get(inst, 0) + 1
    ...
    for inst, ledger in ledgers.items():
        ...
        fills = fill_counts.get(inst, 0)
        avg_pnl_per_fill = (ledger.realized_pnl + unrealized) / fills if fills else Decimal("0")
        avg_fee_per_fill = ledger.fees_paid / fills if fills else Decimal("0")
        ...
        summary[inst] = {
            ...
            "fills": str(fills),
            "avg_pnl_per_fill": str(avg_pnl_per_fill),
            "avg_fee_per_fill": str(avg_fee_per_fill),
        }
...
def _filter_pairs(symbols: str | None) -> list[PairConfig]:
    if not symbols:
        return list(DEFAULT_PAIRS)
    wanted = {s.strip().upper() for s in symbols.split(",") if s.strip()}
    return [p for p in DEFAULT_PAIRS if p.symbol in wanted]
...
def _apply_overrides(
    ...
    guard_threshold_bps: Decimal | None,
    spread_bps: Decimal | None,
    min_profit_bps: Decimal | None,
    ofi_max_bps: Decimal | None,
    max_position_qty: Decimal | None,
    internal_price_delta_limit: Decimal | None,
    refresh_interval_ms: int | None,
) -> list[PairConfig]:
    if not symbol:
        return pairs
    symbol = symbol.upper()
    updated: list[PairConfig] = []
    for pair in pairs:
        ...
        updated.append(
            PairConfig(
                symbol=pair.symbol,
                order_qty=order_qty if order_qty is not None else pair.order_qty,
                min_order_qty=min_order_qty if min_order_qty is not None else pair.min_order_qty,
                max_position_qty=max_position_qty if max_position_qty is not None else pair.max_position_qty,
                guard_threshold_bps=guard_threshold_bps
                if guard_threshold_bps is not None
                else pair.guard_threshold_bps,
                spread_bps=spread_bps if spread_bps is not None else pair.spread_bps,
                min_profit_bps=min_profit_bps if min_profit_bps is not None else pair.min_profit_bps,
                refresh_interval_ms=refresh_interval_ms
                if refresh_interval_ms is not None
                else pair.refresh_interval_ms,
                refresh_offset_ms=pair.refresh_offset_ms,
                liquidity_high_qty=pair.liquidity_high_qty,
                liquidity_low_qty=pair.liquidity_low_qty,
                ofi_enabled=pair.ofi_enabled,
                ofi_max_bps=ofi_max_bps if ofi_max_bps is not None else pair.ofi_max_bps,
                min_quote_lifetime_ms=pair.min_quote_lifetime_ms,
                internal_price_delta_limit=internal_price_delta_limit
                if internal_price_delta_limit is not None
                else pair.internal_price_delta_limit,
            )
        )
    return updated
...
def build_engine(
    ...
    trader_id: str | None,
    max_drawdown_pct_override: Decimal | None,
    daily_loss_limit_override: Decimal | None,
) -> tuple[BacktestEngine, dict[str, CurrencyPair], dict[str, CurrencyPair]]:
    config = BacktestEngineConfig(
        trader_id=TraderId(trader_id or "BACKTEST-LLMMV3-PRIMER"),
    ...
    for pair in pairs:
        ...
        strategy = LeadLagMMv4Primer(
            config=LeadLagMMv4PrimerConfig(
                ...
                max_drawdown_pct=(
                    max_drawdown_pct_override
                    if max_drawdown_pct_override is not None
                    else Decimal("0.10")
                ),
                daily_loss_limit_usdt=(
                    daily_loss_limit_override
                    if daily_loss_limit_override is not None
                    else Decimal("200")
                ),
            )
        )
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--override-order-qty", type=Decimal, default=None)
    parser.add_argument("--override-min-order-qty", type=Decimal, default=None)
    parser.add_argument("--override-guard-threshold-bps", type=Decimal, default=None)
    parser.add_argument("--override-spread-bps", type=Decimal, default=None)
    parser.add_argument("--override-min-profit-bps", type=Decimal, default=None)
    parser.add_argument("--override-ofi-max-bps", type=Decimal, default=None)
    parser.add_argument("--override-max-position-qty", type=Decimal, default=None)
    parser.add_argument("--override-internal-price-delta-limit", type=Decimal, default=None)
    parser.add_argument("--override-refresh-interval-ms", type=int, default=None)
    parser.add_argument("--override-max-drawdown-pct", type=Decimal, default=None)
    parser.add_argument("--override-daily-loss-limit-usdt", type=Decimal, default=None)
    parser.add_argument("--run-id", default=None, help="Optional run identifier for trader_id")
...
def main() -> None:
    args = parse_args()
    _apply_profile(args)
    pairs = _filter_pairs(args.symbols)
    pairs = _apply_overrides(
        pairs,
        args.override_symbol,
        args.override_order_qty,
        args.override_min_order_qty,
        args.override_guard_threshold_bps,
        args.override_spread_bps,
        args.override_min_profit_bps,
        args.override_ofi_max_bps,
        args.override_max_position_qty,
        args.override_internal_price_delta_limit,
        args.override_refresh_interval_ms,
    )
    leader_qdb = None
    follower_qdb = None
    if args.questdb:
        leader_qdb = QuestDbConfig(
            host=args.questdb_host,
            port=args.questdb_port,
            table=args.leader_table or args.questdb_table,
            step_seconds=args.questdb_step_seconds,
        )
        follower_qdb = QuestDbConfig(
            host=args.questdb_host,
            port=args.questdb_port,
            table=args.follower_table or args.questdb_table,
            step_seconds=args.questdb_step_seconds,
        )
    ...
    if args.use_bybit_balance:
        assets = ["USDT"] + [pair.symbol.replace("USDT", "") for pair in pairs]
    ...
    engine, leader_instruments, follower_instruments = build_engine(
        ...
        taker_fee,
        starting_balances,
        f"BACKTEST-LLMMV3-PRIMER-{args.run_id}" if args.run_id else None,
        args.override_max_drawdown_pct,
        args.override_daily_loss_limit_usdt,
    )
