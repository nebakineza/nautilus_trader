...
from strategy.lead_lag_bybit_binance_mm_v001 import LeadLagMM, LeadLagMMConfig
...
def build_engine() -> tuple[BacktestEngine, CurrencyPair, CurrencyPair, LeadLagMM]:
...
def run_once(date_str: str, max_updates: int, data_dir: Path | None, depth: int) -> None:
    engine, leader_instrument, follower_instrument, strategy = build_engine()
    start = time.time()
    leader_count = load_orderbook_data(engine, leader_instrument, date_str, max_updates, data_dir, depth)
    follower_count = load_orderbook_data(engine, follower_instrument, date_str, max_updates, data_dir, depth)
...
def main() -> None:
    ...
    if args.sweep:
        for i in range(args.runs):
            ...
            print("=" * 60)
            run_once(args.date, max_updates, data_dir, args.depth)
    else:
        run_once(args.date, args.max_updates, data_dir, args.depth)
