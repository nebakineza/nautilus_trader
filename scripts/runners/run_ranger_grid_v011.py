"""Run Ranger Grid MM v011 (low-frequency wide grid)."""
...
def main() -> None:
    ...
    strategy_config = RangerGridMMConfig(
        ...
        fixed_lower_price=6.0,
        fixed_upper_price=14.0,
        total_grids=40,
        order_qty=1.0,
    ...
    node.build()
    print(f">>> STARTING {SYMBOL} Ranger Grid v011")
    try:
        node.run()
    finally:
        node.dispose()
