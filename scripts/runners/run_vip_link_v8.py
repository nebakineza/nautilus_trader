"""Run LINK with v008 TIMEKEEPER - Inventory Age Management.
...
def main():
    ...
    # LINK: Was marginal (+15.1 bps) - use wider spread + faster exit
    strategy_config = LeadLagMMv8TimekeeperConfig(
        ...
        min_order_qty=0.5,
        spread_bps=90.0,  # Wider for marginal performer
        inventory_max_hold_secs=1200,  # 20 min max (faster exit)
        inventory_exit_skew_enabled=True,
        inventory_exit_skew_start_secs=180,  # Start tightening after 3 min
        inventory_exit_skew_max_tighten_bps=50.0,
        inventory_underwater_exit_enabled=True,
        inventory_underwater_threshold_bps=-40.0,  # Tighter threshold
        inventory_underwater_check_after_secs=300,  # 5 min
        entry_quality_filter_enabled=True,
        entry_quality_max_trend_strength=0.40,  # Stricter entry
