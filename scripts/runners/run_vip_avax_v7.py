"""Run AVAX with v007 FORTRESS - AGGRESSIVE Profile.

AVAX showing +33.5 bps avg - performing well, keep aggressive.
"""
...
SYMBOL = "AVAXUSDT"
STRATEGY_ID = "VIP-AVAX-V7"
BYBIT_API_KEY = os.environ["BYBIT_API_KEY_MAINACC_03"]
BYBIT_API_SECRET = os.environ["BYBIT_API_SECRET_MAINACC_03"]
...
def main():
    ...
    # AVAX: +33.5 bps avg - AGGRESSIVE profile (performing well)
    strategy_config = LeadLagMMv7FortressConfig(
        ...
        min_order_qty=0.5,
        # === SPREAD (SAFE - increased from 70 to 80) ===
        spread_bps=80.0,
        # === REGIME ===
    ...
    node.build()
    print(f">>> STARTING {SYMBOL} v007 FORTRESS (AGGRESSIVE: 70bps)")
    try:
        node.run()
    finally:
        node.dispose()
