"""Run ETHUSDT Fixed Grid Bot v010b on SG01 subaccount.

Same account as BTC Fixed Grid. Separate process, shared balance (50/50 split).
ETH grid: $1500-$4000, 50 levels, ~1.8% per step, 0.004 ETH per order (~$9.40).
"""
...
def main() -> None:
    ...
    strategy_config = FixedGridBotConfig(
        ...
        total_grids=50,
        fixed_lower_price=1500.0,
        fixed_upper_price=4000.0,
        order_qty=0.004,          # ~$9.40 at $2350
        max_position_qty=0.05,    # ~$118 max exposure
        min_order_value_usd=5.0,
        quote_budget_pct=0.50,    # Share account with BTC grid
    ...
    node.build()
    print(f">>> STARTING {SYMBOL} Fixed Grid Bot v010b (SG01 account)")
    try:
        node.run()
    finally:
        node.dispose()
