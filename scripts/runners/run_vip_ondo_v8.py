"""Run ONDO with v008 TIMEKEEPER - Inventory Age Management.

ONDO - MAINACC_07 - Ultra safe
"""
...
SYMBOL = "ONDOUSDT"
STRATEGY_ID = "VIP-ONDO-V8"
BYBIT_API_KEY = os.environ["BYBIT_API_KEY_MAINACC_07"]
BYBIT_API_SECRET = os.environ["BYBIT_API_SECRET_MAINACC_07"]
...
def main():
    ...
    # ONDO: Was marginal in v6 - keep ultra safe
    strategy_config = LeadLagMMv8TimekeeperConfig(
        ...
        min_order_qty=3.0,
        spread_bps=120.0,  # Ultra wide
        inventory_max_hold_secs=1200,  # 20 min
        inventory_exit_skew_enabled=True,
        inventory_exit_skew_start_secs=180,
        inventory_exit_skew_max_tighten_bps=50.0,
        inventory_underwater_exit_enabled=True,
        inventory_underwater_threshold_bps=-40.0,
        inventory_underwater_check_after_secs=300,
        entry_quality_filter_enabled=True,
        entry_quality_max_trend_strength=0.35,
    ...
    node.build()
    print(f">>> STARTING {SYMBOL} v008 TIMEKEEPER (ULTRA SAFE)")
    try:
        node.run()
    finally:
        node.dispose()
