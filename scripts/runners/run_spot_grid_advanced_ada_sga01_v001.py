"""Run ADAUSDT SpotGridAdvanced v001 on SGA01 subaccount.

Same account as XRP SpotGridAdvanced. Separate process, shared balance (50/50 split).
ADA profile: Large-cap L1, moderate volatility, similar to XRP.
"""
...
SYMBOL = "ADAUSDT"
STRATEGY_ID = "SPOTGRIDADV-ADA-SGA01-V001"
...
def main() -> None:
    ...
    strategy_config = SpotGridAdvancedConfig(
        ...
        trading_limit=25.0,        # ~35 ADA per buy at $0.72
        tl_multiplier=1.0,
        max_buy_count=15,
        ...
        funds_reserve=0.0,
        keep_quote=0.0,
        quote_budget_pct=0.50,     # Share account with XRP
        period="15m",
        gain_pct=0.5,
        auto_gain=True,
        fee_bps=20.0,
    ...
    node.build()
    print(f">>> STARTING {SYMBOL} SpotGridAdvanced v001 (SGA01 account)")
    try:
        node.run()
    finally:
        node.dispose()
