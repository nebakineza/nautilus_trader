"""Run NEAR with v008 TIMEKEEPER - Inventory Age Management.
...
SYMBOL = "NEARUSDT"
STRATEGY_ID = "VIP-NEAR-V8"
BYBIT_API_KEY = os.environ["BYBIT_API_KEY_MAINACC_05"]
BYBIT_API_SECRET = os.environ["BYBIT_API_SECRET_MAINACC_05"]
...
def main():
    ...
    # NEAR: Lost in v7 (-36 bps) - faster exit, wider spread
    strategy_config = LeadLagMMv8TimekeeperConfig(
        ...
        min_order_qty=5.0,
        spread_bps=90.0,  # Widen for loser
        inventory_max_hold_secs=1200,  # 20 min max (faster)
        inventory_exit_skew_enabled=True,
        inventory_exit_skew_start_secs=180,  # 3 min
        inventory_exit_skew_max_tighten_bps=50.0,
        inventory_underwater_exit_enabled=True,
        inventory_underwater_threshold_bps=-40.0,  # Tighter
        inventory_underwater_check_after_secs=300,  # 5 min
        entry_quality_filter_enabled=True,
        entry_quality_max_trend_strength=0.40,  # Stricter
    ...
    node.build()
    print(f">>> STARTING {SYMBOL} v008 TIMEKEEPER")
    try:
        node.run()
    finally:
        node.dispose()
