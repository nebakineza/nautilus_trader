"""Run AVAXUSDT Ranger Grid MM v011 on RG02 subaccount.

Standalone account ($300 USDT). AVAX profile matches LINK: ~$9, 4% daily vol,
4.5x oscillation ratio. Dynamic ATR-based range adapts as AVAX trends.
"""
...
SYMBOL = "AVAXUSDT"
STRATEGY_ID = "RANGER-AVAX-V011"
BYBIT_API_KEY = os.environ["BYBIT_API_KEY_RG02_00"]
BYBIT_API_SECRET = os.environ["BYBIT_API_SECRET_RG02_00"]
...
def main() -> None:
    ...
    strategy_config = RangerGridMMConfig(
        ...
        fixed_lower_price=5.0,
        fixed_upper_price=15.0,
        total_grids=40,
        order_qty=1.0,            # ~$9.11 per order
        max_position_qty=20.0,    # ~$182 max exposure
        min_order_value_usd=5.0,
        quote_budget_pct=1.0,     # Standalone account (100%)
    ...
    node.build()
    print(f">>> STARTING {SYMBOL} Ranger Grid v011 (RG02 account)")
    try:
        node.run()
    finally:
        node.dispose()
