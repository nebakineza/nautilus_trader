"""Run LINK v010 Scalper (Lead-Lag)."""
...
from lead_lag_bybit_binance_scalper_v010 import LeadLagScalper, LeadLagScalperConfig

SYMBOL = "LINKUSDT"
STRATEGY_ID = "SCALPER-LINK-V010"
...
def main() -> None:
    ...
    # Reduced sizing for current balance
    strategy_config = LeadLagScalperConfig(
        ...
        momentum_window=3,
        entry_momentum_bps=2.0,
        entry_ofi_threshold=0.12,
        entry_requires_both=True,
        long_only=True,
        ...
        taker_fee_bps=10.0,
        min_edge_bps=2.0,
        take_profit_tif=TimeInForce.GTC,
        take_profit_post_only=True,
        max_hold_ms=5000,
        cooldown_ms=1000,
    ...
    node.trader.add_strategy(LeadLagScalper(config=strategy_config))
    node.add_data_client_factory(BYBIT, BybitLiveDataClientFactory)
    node.add_exec_client_factory(BYBIT, BybitLiveExecClientFactory)
    node.add_data_client_factory("BINANCE_SPOT", BinanceLiveDataClientFactory)
    node.build()
    print(f">>> STARTING {SYMBOL} v010 SCALPER")
    try:
        node.run()
    finally:
        node.dispose()
